...
# 三件套通用协议（trio-protocol · standard-v3）

> 本文件由 `/project-init` 创建。**所有遵循 `trio: standard-v3` 的项目共享这套协议**，子项目通过相对路径 `../docs/trio-protocol.md` 引用最近祖先的副本。
>
> 顶部 `protocol-version` 是机器可读版本号，`/project-init` detect 用它判断副本是否落后于当前标准。
>
...
> 进度只走这三份：「做了什么」进 CHANGELOG，「下一步」进 INDEX Handoff。**不再维护独立的非工程进度页**（v2 的 PROJECT_PROGRESS.md 已于 v3 退役）。
...
拆 `### 概述` + `### 明细` 两个子段。**概述**是一句话级的下一步（看板类消费方 App 只抓这段当 next/blocked_by）；**明细**是给人/agent 看的展开说明，App 不读。
...
### 明细
<YYYY-MM-DD：展开说明、关键命令 / 文件指针、上下文；详见 obwiki <wiki page>。>
```

> 概述写**纯文本加粗行**（每行一条，列表 `- ` 前缀可选）：非 `⚠/阻塞` 前缀归 next，`⚠`/`阻塞` 前缀归 blocked_by。无 `### 概述` 子标题时按旧单段写法向后兼容。
...
提醒话术示例：
> 这一段告一段落了。我会把 CHANGELOG 加一条检索记录，并更新 INDEX 接力点。这一阶段的改动清单：...
...
*本协议由 `/project-init` 维护，版本 `standard-v3`（`protocol-version: 3`）。规则演化登记在 `project-init/references/migrations.md`，老项目通过 `/project-init migrate` 按需升级。*
