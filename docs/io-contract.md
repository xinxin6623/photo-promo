...
│   └── ...
└── images/
    └── <id>.<ext>        # 入库原图（L2）；默认复制（config export.copy_images，可改软链）
```
`<id>`：图片内容 sha256 前 12 位（稳定、去重友好；位数见 config export.id_len）。
...
- `related` 列同一活动/场景/人物的图 id，每条带一句"为什么相关"（格式：`<id> — 同场景(合影)，拍摄相差约 N 分钟`）。
- 建链时机：Phase 4 导出时按 scene 相同 且 taken_at 相差 ≤ `export.link_window_min`（默认 30）分钟自动建初版，evolve 阶段精修。
